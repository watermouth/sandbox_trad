...
(* #require "calendar" *)
open CalendarLib 
open Item (* item.ml からItemという名前の構造が定義される. item.ml内でItemという構造を記述しているので, 
             結果としてItem.Item.t という型などを定義したことになる. そこでItemをopenしておかないといけない
             そもそも, item.ml内部で structure を明示的に定義しなければ良いという話ではないか... *)
open Buysell
module Trade = 
  struct
...
let sample1 = Trade.make 1 date1 (Some time1) 1 0 10000 (Some 194.325)
...
let sampleTrades =
  ...
  Array.append
  (Array.init num
    (fun i -> Trade.make i date1 (Some (Time.add (Time.make 7 0 0) (Time.Period.second i)))
              1 (Random.int 2) 10000 (Some 89.321))
  )
  (Array.init num
    (fun i -> Trade.make (i+num) date1 None 1 (Random.int 2) 10000 None) 
  );;

(* show *)
Array.iter (fun x -> Trade.to_string x) sampleTrades
*)
