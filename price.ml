...
let sample1 = Price.make 0 date1 time1 1(*USDJPY*) sample_bid sample_mid sample_ask
;;
Price.to_string sample1;;
...
let make_samples num =
  ...
  (fun i -> let diff = Random.float 0.001 in
    ...
    Price.make i date1 (Time.add time1 (Time.Period.second i)) 2 
    b ((b +. a) /. 2.) a
  );;

Price.from_array_to_string (make_samples 10)
